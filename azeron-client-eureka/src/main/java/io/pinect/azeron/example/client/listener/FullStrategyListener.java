...
@Log4j2
@AzeronListener(eventName = "full_event_name", ofClass = SimpleAzeronMessage.class, policy = HandlerPolicy.FULL)
public class FullStrategyListener implements SimpleEventListener<SimpleAzeronMessage> {
    private final String serviceName;

    @Autowired
    public FullStrategyListener(@Value("${spring.application.name}") String serviceName) {
