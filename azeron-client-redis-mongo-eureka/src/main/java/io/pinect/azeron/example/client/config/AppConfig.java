...
@Configuration
@EnableMongoRepositories(basePackages = {"io.pinect.azeron.example.client.azeron.domain.repository"})
public class AppConfig {
    ...
    public SimpleMongoDbFactory mongoDbFactory(){
        ...
        return new SimpleMongoDbFactory(new MongoClient("127.0.0.1", settings), "azeronClientExampleDb");
    }
    ...
    public MongoTemplate mongoTemplate() {
        ...
        mongoTemplate.indexOps(MongoAzeronMessageEntity.class).ensureIndex(new Index().on("seenSubscribers", Sort.Direction.DESC).named("azeron_message_seenSubscribers"));
        mongoTemplate.indexOps(MongoAzeronMessageEntity.class).ensureIndex(new Index().on("subscribers", Sort.Direction.DESC).named("azeron_message_subscribers"));
